...
if __name__ == '__main__':
    ...
    global_ops = "D:/毕业设计/picture/"

    '''
    # 原始数据集
    data_ops = "D:/vcdb_new_2019.csv"    
    data_incident = idf_read_csv(data_ops)
    
    # 初步数据处理（缺失值处理）
    new_ops = "D:/vcdb_new_2019_drop_miss.csv"
    drop_new_data = drop_miss_data(data_incident, new_ops)
    
    # 安全事件数据集
    all_incident_data_name = global_ops + "all_incident" + ".csv"
    ...
    all_breach_data_name = global_ops + "all_breach" + ".csv"
    data_breach.to_csv(all_breach_data_name, index = None)
    '''

    incident_ops = global_ops + "all_incident.csv"
